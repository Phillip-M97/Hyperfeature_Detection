...
def find_nn_source_correspondences(img1_feats, img2_feats, source_points, output_size, load_size):
    ...
    print (f"Shape of img1_feats after selecting points: {img1_feats.shape}")
    print (f"Shape of img2_feats: {img2_feats.shape}")


    img1_feats = normalize_feats(img1_feats)
    img2_feats = normalize_feats(img2_feats)
    print(f"img1_feats_shape {img1_feats.shape}, img2_feats_shape {img2_feats.shape}")
    sims = torch.matmul(img1_feats, img2_feats.permute((0, 2, 1)))
    print(f"Original Sims {sims}, shape = {sims.shape}")

    # Find nn_correspondences but with points1 = source_points
    num_pixels = int(math.sqrt(sims.shape[-1]))
    print(f"sims for point calculation: {sims.shape}")
    points2 = sims.argmax(dim=-1)
    print(f"Target Points after sims.argmax {points2}, shape = {points2.shape}")
    ...
    points2 = points2[0]
    print(f"Input points type: {type(points1)}, shape {points1.shape}")
    print(f"Predicted points type: {type(points2)}, shape {points2.shape}")

    return points1, points2

def find_nn_multisource_correspondences2(source_imgs_feats, target_img_feats, source_points_list, load_size, device):
    load_size = tuple(load_size) if isinstance(load_size, list) else load_size

    target_img_feats = torch.nn.functional.interpolate(target_img_feats, load_size, mode="bilinear", align_corners=False)
    target_img_feats = flatten_feats(target_img_feats)
    target_img_feats = normalize_feats(target_img_feats)

    print(f"Source_Points \n {source_points_list}")

    max_sims = None
    best_points = None

    for idx, (img1_feats, source_points) in enumerate(zip(source_imgs_feats, source_points_list)):

        print(f"Processing source image {idx+1}/{len(source_imgs_feats)}")

        img1_feats = torch.nn.functional.interpolate(img1_feats, load_size, mode="bilinear", align_corners=False)
        img1_feats = flatten_feats(img1_feats)
        img1_feats = normalize_feats(img1_feats)
        ...
        source_idx = torch.from_numpy(points_to_idxs(source_points, load_size)).long().to(device)
        img1_feats = img1_feats[:, source_idx, :]

        print(f"Shape of img1_feats after selecting points: {img1_feats.shape}")


        sims = torch.matmul(img1_feats, target_img_feats.permute((0, 2, 1)))
        ...
        if max_sims is None:
            ...
            best_points = torch.where(better_mask.unsqueeze(-1), sims.argmax(dim=-1), best_points)
            max_sims = torch.where(better_mask.unsqueeze(-1), sims, max_sims)

    num_pixels = int(math.sqrt(sims.shape[-1]))
    print(f"Target Points after sims.argmax {best_points}, shape = {best_points.shape}")

    points_x = best_points % num_pixels
    points_y = best_points // num_pixels
    predicted_points = torch.stack([points_y, points_x], dim=-1)
    
    # Assume all source_points_list entries are structured identically for each source image
    if not isinstance(source_points_list[0], np.ndarray):
        points1 = torch.from_numpy(np.array(source_points_list[0]))
    else:
        points1 = torch.from_numpy(source_points_list[0])
    
    print(f"Input Points {points1}; Predicted Points {predicted_points[0]}")

    print(f"Input points type: {type(points1)}, shape {points1.shape}")
    print(f"Predicted points type: {type(predicted_points[0])}, shape {predicted_points[0].shape}")

    return points1, predicted_points[0]

"""
def find_nn_correspondences_multisource(source_imgs_feats, target_img_feats, source_points, load_size, device):
    target_img_feats = torch.nn.functional.interpolate(target_img_feats, load_size, mode='bilinear').to(device)
    target_img_feats = flatten_feats(target_img_feats)
    target_img_feats = normalize_feats(target_img_feats)

    max_sims = torch.full((len(source_points), target_img_feats.shape[1]), float('-inf'), device = device)

    for img_feats in source_imgs_feats:
        img_feats = torch.nn.functional.interpolate(img_feats, load_size, mode = 'bilinear').to(device)
        img_feats = flatten_feats(img_feats)
        img_feats = normalize_feats(img_feats)

        source_idx = torch.from_numpy(points_to_idxs(np.array(source_points), load_size)).long().to(device)
        selected_source_feats = img_feats[:, source_idx, :]

        # Ensure the selected features have the correct shape for matmul operation
        if selected_source_feats.dim() == 3 and selected_source_feats.shape[0] == 1:
            selected_source_feats = selected_source_feats.squeeze(0)

        # Compute cosine similarity between selected source points and all target points
        sims = torch.matmul(selected_source_feats, target_img_feats.t())
        max_sims = torch.maximum(max_sims, sims)

    points_indices = max_sims.argmax(dim = -1)
    num_pixels = int(math.sqrt(target_img_feats.shape[-1]))
    points_x = points_indices % num_pixels
    points_y = points_indices // num_pixels
    predicted_points = torch.stack([points_y, points_x], dim = -1)

    source_point_tensor = torch.tensor(source_points, dtype = torch.int32, device = device)
    return source_point_tensor, predicted_points
"""
...
def draw_correspondences_multi(source_images_paths, source_points_dict, predicted_points, target_image_path, title=""):
    ...
    for idx, (key, source_image_path) in enumerate(source_images_paths.items()):
        ...
        axs[idx].scatter(points[:, 1], points[:, 0], color='red', label='Source Points')
        axs[idx].legend()
    ...
    axs[-1].scatter(predicted_points[:, 1], predicted_points[:, 0], color='blue', label='Predicted Points')
    axs[-1].legend()
